...
def test_hip_eplb_maps_only_routed_columns_before_shared_slots(
    ...
    monkeypatch.setattr(topk_module, "_use_aiter", False)
    ...
    ids = torch.cat([routed, torch.full((rows, shared), 4)], dim=1)
    ...
    torch.testing.assert_close(actual_weights[:, 2:], torch.full((rows, shared), 0.5))
